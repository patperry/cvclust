...
located, and can easily fail (we demonstrate such a failure in Section~1 of
the supplementary appendix).  In this report, we propose a new method to
...
columns of the data matrix and then partition the rows and columns as $N = n + m$ and $P = p + q$ for positive integers $n$, $m$, $p$, and $q$.  We treat
...
We turn now to theoretical properties of Gabriel cross-validation. In
Section~2 of the supplementary appendix, show that the procedure is
self-consistent, in the sense that it recovers the correct value of $k$ in the
absence of noise.  This gives us some assurance that the procedure is
well-behaved, but we can bolster our confidence and gain insight into its
workings by analyzing its behavior in the presence of noise. To that end, we
devote the present section to analysis of the procedure in the presence of
Gaussian noise.


\subsection{Single cluster in two dimensions}

We first study the case of a single
...
Here and throughout the text, our simulations use
R~software environment~\citep{rproject}, with most figures generated
with the support of the \texttt{ggplot2} library~\citep{ggplot2}.
...
%\ref{sec:single-general-dim}
using similar techniques to those used
...
Lemma~3 from Section~5 of the supplementary appendix
    to compute the conditional expectations;
...
spread across all $P$ features. This ensures that the cluster centers
are well identified.
The second reason for multiplying by $Q$ is to spread any
...
We use the default parameter settings for all competing methods, implented
in the following R~packages:
\texttt{cluster} \citep{cluster};
\texttt{cstab} \citep{cstab};
\texttt{e1071} \citep{e1071};
\texttt{fpc} \citep{fpc};
\texttt{MASS} \citep{MASS};
\texttt{mclust} \citep{mclust};
and
\texttt{NbClust} \citep{NbClust}.
For Gabriel cross-validation,
...
cross-validation, which we describe in section~6 of the supplementary
appendix. %Appendix~\ref{sec:wold-cv}.
...
\begin{figure}
\centering
\includegraphics[width=5.5in, height=2.75in]{main_code/demo/bench/setting1/Facet.pdf}
\caption{Setting 1 performance as correlation increases, faceted by estimation
    method. For each method, the subplot shows the proportion of
...
\begin{figure}
\centering
\includegraphics[width=5.5in, height=2.75in]{main_code/demo/bench/setting2/Facet.pdf}
\caption{Setting 2 performance as the number of noise dimensions increases,
    faceted by estimation method. For each method, the subplot shows the proportion of
    replicates in which estimated number of clusters agrees with the truth
...
\begin{figure}
\centering
\includegraphics[width=5.5in, height=2.75in]{main_code/demo/bench/setting3/Facet.pdf}
\caption{Setting 3 performance as the number of dimensions increases,
    faceted by estimation method. For each method, the subplot shows the proportion of
    replicates in which estimated number of clusters agrees with the truth
    versus the number of noise dimensions,~$r$. Both variants of Gabriel
    cross-validation have perfect accuracy in the replicates
...
\begin{figure}
\centering
\includegraphics[width=5.5in, height=2.75in]{main_code/demo/bench/setting4/Facet.pdf}
\caption{Setting 4 performance as the variance heterogeneity increases,
    ...
    cross-validation have near-perfect accuracy for all values of~$R$.}
\label{fig:setting4}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=5.5in, height=2.75in]{main_code/demo/bench/setting5/Facet.pdf}
\caption{Setting 5 performance as the tail heaviness increases,
    ...
    methods as the tail heaviness increases.}
\label{fig:setting5}
\end{figure}
...
To validate our clusters, in Section~7 of the supplementary appendix we follow
\citet{tavazoie1999systematic} by performing an enrichment analysis to
discover which functional gene groups are significantly over-represented in
each cluster.  We find that Cluster~1 is enriched with genes that somatize
cell stress, such as oxidative heat-induce proteins.  Cluster~2 contains genes
that govern mitochondrial translation and mitochondrion organization.
Cluster~3, the first period cluster, contains cell cycle genes related to
budding and cell polarity, along with genes that govern RNA processing and
transcription. Cluster~4 contains genes related to cytoplasmic translation and
genes encoding ribosomes. Cluster~5, the second periodic cluster, contains
genes that participate cell-cycle processes, along with DNA replication and
DNA repair.


For the clusters that Tavazoie et al.\ were able to characterize, our analysis
broadly agrees with the earlier clustering. We can see this in
Figure~\ref{fig:5_clusters}, which gives us a graphical confusion matrix
comparing the two analyses.  The plot in cell $(i,j)$ of the upper left part
of this figure gives the mean expression level for genes in the intersection
of Tavazoie's Cluster~$i$ and our Cluster~$j$; the plots in the margins give
the mean expression levels for Tavazoie's clusters (top right) and our
clusters (bottom left).  We only include a plot for cell $(i,j)$ if the number
of genes in that cell is greater than $20$.  Section~8 of the supplementary
appendix contains a more detailed comparison.
The major difference between our analysis and that of
\citet{tavazoie1999systematic} is that we are able to identify meaningful
groups of genes with a much smaller value of $k$ ($k = 5$ instead of $k =
30$), and we are able to interpret all of the clusters found by our analysis.
