...
for (s in list.dirs(full.names=FALSE, recursive=FALSE)){
	...
	if(s == "setting5"){
		...
		ggplot(Result_data, aes(X_axis,Proportion, group = Methods, colour = Methods))+ geom_line(size = 1.3)+labs(x = Label_X[[s]])+facet_wrap(~Methods,scales = "free_y",nrow=3)+scale_x_reverse()
		...
		ggplot(Result_data, aes(X_axis,Proportion, group = Methods, colour = Methods))+ geom_line(size = 1.3)+labs(x = Label_X[[s]])+facet_wrap(~Methods,scales = "free_y",nrow=3)
		ggsave(file.path(s,"Facet.pdf"),width = 11, height = 7)
	}
}
