...
##Get 5X30 confusion matrix
Confusion <- matrix(NA,5,30)

for(i in 1:5){
	Subset <- DATA[DATA$Gabriel==i,];
	for(j in 1:30){
		Confusion[i,j] = sum(Subset$Cluster==j)
	}
}
...
##main plot
main <- ggplot(Sum_data3, aes(X, Y))+geom_line(size = 1)+labs(title = "", x = "", y = "")+facet_grid(Tava~Gabriel)+geom_errorbar(aes(ymax = Upper, ymin=Lower),colour = "black")
...
grid.arrange(top, empty, main, right, ncol=2, nrow=2, widths=c(3, 1), heights=c(1, 3))
dev.off()



ggsave("split_plots.pdf",)
