...
\begin{lemma}\label{lem:pollard-continuous}\label{lem:pollard-last}
...
points, then $m_1(P) > m_2(P) > \dotsb > m_k(P)$.  Further, there exists a
finite $M$ such that $W(\cdot, P)$ attains its infimum over $\sA_k$ and each
minimizer $\bar A$ is contained in the set
\(
  \sE_k = \{ A \in \sA_k : A \subset B(5M) \},
\)
where $B(5M)$ denotes the closed ball of radius~$5M$.
\end{lemma}
\begin{proof}
...
\)
Let $\sC_1$ denote the class of sets $A$ with at most $k$ points and at least
one point in in $B(M)$.  Let $A_0$ be the singleton set containing the origin.
If no point of $A$ is in $B(M)$, then
\(
  W(A, P) \geq (M - r)^2 P(K) > W(A_0, P).
\)
Put concisely, if $A \in \sA_k \setminus \sC_1$, then
\(
...
minimizers lie in $\sE_k$.


We first show that if $M$ is large enough, then there exists a set $A \in \sE_k$
and a positive $\varepsilon$ with $\varepsilon + W(A,P) < m_{k-1}(P)$.
Since the support of $P$ contains least $k$ points, there exists an $x_0$ not
in $\bar A_{k-1}$ that lies in the support of $P$.  Ensure that $M$ is large
enough so that $x_0 \in B(5M)$.  Take $A \in \sE_k$ to be the set of $k$
points gotten by adjoining $x_0$ to $\bar A_{k-1}$.  Let
\(
  \delta = \min_{a \in \bar A_{k-1}} \| x_0 - a \|
\)
and set $R = \{ x \in \mathbb{R}^d : \|x - x_0\| < \delta/3 \}$; since $x_0$
is in the support of $P$, the neighborhood $R$ has positive $P$~measure.
Further,
\[
  \int_{R} \min_{a \in \bar A_{k-1}} \|x - a\|^2 P(dx)
  \geq
  \int_{R} \min_{a \in \bar A_{k-1}} (\|x_0 - a\| - \delta/3)^2 P(dx)
  = (4/9) \delta^2 P(R),
\]
and
\[
  \int_{R} \min_{a \in A} \|x - a\|^2 P(dx)
  =
  \int_{R} \|x - x_0\|^2 P(dx)
  \leq
  (1/9) \delta^2 P(R).
...
\(
  m_{k-1}(P) - W(A, P) = 
  W(\bar A_{k-1}, P) - W(A, P) > \varepsilon.
\)
...
\label{lem:pollard-first}
Suppose that $\int \|x\|^2 P(dx) < \infty$.  If $A_n$ minimizes $W(\cdot,
P_n)$, then, there exists a finite $M$ such that as if $n$ is large enough,
...
\begin{proof}
If $P$ has finite support, then the result follows immediately.  For the
remainder of the proof, then, we will assume that $P$ has infinite support.

In Section~3 of his paper, Pollard proves the result, assuming that there
is a unique set of $j$ points minimizing $W(\cdot, P)$ for each value of
$j = 1, \dotsc, k$.  His proof does not use this assumption directly, only
its implication that for each $j$ in the range $2, \dotsc, k$, there exists
a set with $j$ points satisfying $W(A,P) < m_{j-1}(P)$.  If we can establish the
implication, then the rest of his proof will apply.

We will prove the implication by construction.  Take $j$ to be in the range
$2, \dotsc, k$.  For any $\varepsilon > 0$, there exists a set
$B_{\varepsilon}$ containing at most $j-1$ points such that
$W(B_{\varepsilon}, P) < m_{j-1}(P) + \varepsilon$.  We will add a single
point to $B_{\varepsilon}$ to get a set $A_{\varepsilon}$ with $j$ or fewer
points satisfying $W(A, P) < m_{j-1}(P)$.

If we can show that $m_{j+1}(P) > m_{j}(P)$ for $j = 1, \dotsc, k-1$, then
we will have established the conditions required for Pollard's proof of the
analogous result in Section 3 of his paper.

\end{proof}


\begin{lemma}\label{lem:polalrd-uniform}
...
\end{lemma}
\begin{proof}
Given at the beginning of Pollard's Section~4.
\end{proof}
