...
Our proof
relies on results from \citet{pollard1981strong}, who analyzed $k$-means and
showed that the within-cluster sum of squares converges to a population
quantity.  Specifically, for any finite set $A \subset \mathbb{R}^d$ and
probability measure $P$ supported on $\mathbb{R}^d$, define the population
...
Set
\[
  m_k(P) = \inf\{ W(A, P) : \text{$A$ contains $k$ or fewer points} \}.
\]
Suppose that $X_1, \dotsc, X_n$ are independent draws from $P$, and
if $P_n$ is the empirical measure formed by placing mass $1/n$ on each $X_i$.
Let $A_n$ be a set minimizing $W(A_n, P_n)$.  Pollard gave conditions for
$A_n$ to converge to the minimizer of $W(\cdot, P)$.  His main theorem assumes that
the minimizer of $W(\cdot, P)$ is unique, which is too strong for our purposes
(this fails, for example, with a bivariate normal distribution when $k \geq 2$).
It is straightforward to generalize his result to cases with multiple
maximizers.  To do so, we will need the following three lemmas.  The proofs of
these lemmas are given in Sections~3 and~4 of Pollard's article.



\begin{lemma}\label{lem:pollard-compact}\label{lem:pollard-first}
Suppose that $\int \|x\|^2 P(dx) < \infty$ and that
$m_j(P) > m_{j+1}(P)$ for $j = 1, \dotsc, k-1$.  If $A_n$ minimizes
$W(\cdot, P_n)$, then, there exists a finite
$M$ such that as if $n$ is large enough, then, almost surely, $A_n$ is in
the class of sets $\sE_k = \{ A \subset B(5M) : \text{$A$ contains $k$ or fewer
points} \}$, where $B(5M)$ denotes the Euclidean ball of radius $5M$ centered
at the origin.
\end{lemma}
\begin{proof}
Follows from Pollard's argument in Section 3 of his paper.
\end{proof}
...
If $\int \|x\|^2 P(x) < \infty$, then
the map $A \mapsto W(A, P)$ is continuous on $\sE_k$ with respect to the
topology induced by the Hausdoff metric $H(\cdot, \cdot)$, defined for compact
subsets $A, B$ of $\mathbb{R}^d$ by $H(A,B) < \delta$ if and only if every
point of $A$ is within Euclidean distance $\delta$ of at least one point of
$B$, and vice versa.
\end{lemma}
\begin{proof}
Given at the end of Pollard's Section~4.
\end{proof}
