...
existing methods as well as the proposed Gabriel cross-validation method and Wold method. Higher correlation clearly 
degenerate most algorithms' ability to find the correct $k$ as data no longer spherical in $p$-dimension.
And the recent proposed methods (Gabriel cross-validation method, Gap statistic, Jump method) seem to be more sensitive to
...
proposed Gabriel cross-validation method as well as its correlation-corrected version actually working better in higher 
dimension. 

Result of setting $4$ indicates most previously methods are sensitive to variance heterogeneity among 
clusters, most notably the Gap statistics and model-based BIC method. The
proposed Gabriel cross-validation method
...
In conclusion, the proposed Gabriel cross-validation method compares well with current state-of-the-art methods and 
is very robust against variance heterogeneity, high dimension and heavy-tail data. One weakness of the 
Gabriel cross-validation method is that it's sensitive to the high correlation among dimensions. In such case, its 
...
\section{Application on Yeast data set}
...
\section{Wold cross-validation estimation} \label{app:foobar}
\begin{itemize}
