...
``responses''; similarly, we treat the first $n$ rows as ``training'' and the
last $m$ rows as ``test''.  In block form, the permuted data matrix is
\[
  \dataX
  =
  \begin{bmatrix}
...
TODO: describe theoretical support
...
gives conditions for Gabriel CV to recover the true number of clusters in the
absence of noise.
...
The result statement of Theorem~\ref{thm:self-consistency} follows from 
...
cross-validation breaks down in the presense of strong correlation.
