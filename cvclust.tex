...
cross-validation procedure.  As in section~\ref{sec:gabriel-cv-algorithm}
...
Given $\mu(g) = \bigl(\muX(g),\muY(g)\bigr)$ are distinct for $g = 1, \dotsc, K$,
~\ref{asn:self-consistency-distinct-mux} and ~\ref{asn:self-consistency-distinct-muy}
are reasonable given not too small dimension $p$ and $q$.
...
\)
\end{proof}
The inequality in Proposition~\ref{prop:twoclust} means that the proposed method
would correctly pick $k = 2$ given that $(\muX, \muY)$ and 
$(-\muX, -\muY)$ are not too close. 
