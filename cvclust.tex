...
We have stated and prooved Proposition~\ref{prop:twoclust} for data in $d = 2$
...
over-estimate the true number of clusters, our covariance estimate $\hat
\Sigma$ will often be biased. Fortunately, we do not need to estimate $\Sigma$
with high accuracy. For our correction to be useful, we only need $\hat
\Sigma^{-1/2}$ to have lower within-cluster correlation than the raw data. In
our simulations and in our empirical validation study in
Sections~\ref{sec:simulations} and~\ref{sec:empirical-validation} we
demonstrate that despite its theoretical shortcoming, our ad-hoc correlation
