...
In practice, one can check such condition using estimated sample covariance matrix for single cluster case. 
If such condition doesn't met, one solution is rotating the matrix to eliminate the correlation between each dimension then perform Gabriel CV method on the rotated data matrix. Another empirically observed solution is 
to leave most columns for clustering, i.e. increase the dimension of $\Sigma_{YY}$. Such solution is especially 
useful for data containing multiple clusters, where neither estimating sample covariance matrices nor
rotating to eliminate correlation is feasible. Although the theory for such solution is available for special cases,
